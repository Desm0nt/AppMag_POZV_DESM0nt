...
{
    /// <summary>
    /// Static mechanic finite element solution (optimized)
    /// </summary>
    public class StaticMechanicOptimizedSolution : ISolution
    {
        ...
        public StaticMechanicOptimizedSolution(ISolve<double[,]> solver)
        {
            this.solver = solver;
            ...
            foreach (var item in conditions.FixedNodes)
            {
                switch (type)
                {
                    case TypeOfFixation.RIGID:
                        ...
                        globalMatrix[item.Key * DEGREES_OF_FREEDOM, 0] = 1.0;
                        MathOps.SetZerosRow(ref globalMatrix, item.Key * DEGREES_OF_FREEDOM + 1);
                        globalMatrix[item.Key * DEGREES_OF_FREEDOM + 1, 0] = 1.0;
                        MathOps.SetZerosRow(ref globalMatrix, item.Key * DEGREES_OF_FREEDOM + 2);
                        globalMatrix[item.Key * DEGREES_OF_FREEDOM + 2, 0] = 1.0;
